...
No external dependencies required for the local list.
The async API validation below uses aiohttp (already added to requirements).
"""
...
def _looks_like_word_entry(data):
    """Whether an API body plausibly contains dictionary data."""
    # dictionaryapi.dev returns a list of entries
    if isinstance(data, list):
        return len(data) > 0
    # freedictionaryapi.com returns an object describing the word
    if isinstance(data, dict):
        return bool(
            data.get("word")
            or data.get("entries")
            or data.get("meanings")
            or data.get("definitions")
        )
    return False


async def _fetch_dictionary(url, label):
    """
    Hit one dictionary API. Returns WordStatus.VALID / INVALID / ERROR.

    200       -> valid when the body holds dictionary data
    404       -> word confirmed to not exist
    anything else / network trouble / bad JSON -> ERROR
    """
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.get(url) as resp:
                ...
                if code == 200:
                    ...
                    if _looks_like_word_entry(data):
                        return WordStatus.VALID
                    print(f"[WORDCHAIN] {label} dictionary API unexpected 200 payload")
                    return WordStatus.ERROR
...
async def _api_validate(word):
    ...
    A word is:
      VALID   -> either API confirms it exists
      INVALID -> at least one API authoritively says it does not exist
      ERROR   -> neither API could give a definitive answer
    """
    encoded = quote(word, safe="")

    primary = await _fetch_dictionary(PRIMARY_API_URL.format(encoded), "primary")
    if primary == WordStatus.VALID:
        return WordStatus.VALID

    fallback = await _fetch_dictionary(FALLBACK_API_URL.format(encoded), "fallback")
    if fallback == WordStatus.VALID:
        return WordStatus.VALID

    if primary == WordStatus.INVALID or fallback == WordStatus.INVALID:
        return WordStatus.INVALID
